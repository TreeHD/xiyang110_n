...
// udpSessionManager 管理所有UDP会话
var udpSessionManager = struct {
	sync.RWMutex
...
func init() {
	go func() {
		for {
			...
			now := time.Now()
			udpSessionManager.Lock()
			for key, session := range udpSessionManager.sessions {
				if now.Sub(session.lastActive) > udpSessionTimeout {
					...
					delete(udpSessionManager.sessions, key)
				}
			}
			udpSessionManager.Unlock()
		}
	}()
}
...
func handleUDPProxy(ch ssh.Channel, remoteAddr net.Addr) {
	...
	defer func() {
		...
		udpSessionManager.Lock()
		if session, ok := udpSessionManager.sessions[clientKey]; ok {
			session.publicConn.Close()
			delete(udpSessionManager.sessions, clientKey)
		}
		udpSessionManager.Unlock()
		ch.Close()
	}()
	...
	udpSessionManager.Lock()
	udpSessionManager.sessions[clientKey] = session
	udpSessionManager.Unlock()
...
func tcpToUDP(s *fullConeSession) {
	...
	for {
		...
		case 0x01: // IPv4
			...
			if _, err := io.ReadFull(s.clientChannel, addrBytes); err != nil {
				return
			}
			host = net.IP(addrBytes).String()
		case 0x03: // Domain
			lenBytes := make([]byte, 1)
			if _, err := io.ReadFull(s.clientChannel, lenBytes); err != nil {
				return
			}
			domainBytes := make([]byte, lenBytes[0])
			if _, err := io.ReadFull(s.clientChannel, domainBytes); err != nil {
				return
			}
		...
		case 0x04: // IPv6
			...
			host = fmt.Sprintf("[%s]", net.IP(addrBytes).String())
		default:
			log.Printf("FullCone NAT: Unsupported ATYP %d from %s", header[3], clientKey)
			return
		}

		portBytes := make([]byte, 2)
		if _, err := io.ReadFull(s.clientChannel, portBytes); err != nil {
			return
		}
		...
		if _, err := io.ReadFull(s.clientChannel, dataLenBytes); err != nil {
			return
		}
		...
		if _, err := io.ReadFull(s.clientChannel, buf); err != nil {
			return
		}
