...
func apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	case strings.HasSuffix(r.URL.Path, "/status") && r.Method == "PUT":
		...
		if err := safeSaveConfig(); err != nil { http.Error(w, `{"message":"保存配置文件失败"}`, http.StatusInternalServerError); return }; sendJSON(w, http.StatusOK, map[string]string{"message": fmt.Sprintf("账户 %s 状态更新成功", username)})
