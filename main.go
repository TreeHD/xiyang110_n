...
// --- 核心数据转发逻辑 [移植自您验证过的历史版本] ---

// copyTracker 负责流量统计（您可以根据需要添加流量计数的逻辑）
...
func (c *copyTracker) Write(p []byte) (n int, err error) {
	...
	// 可以在这里添加原子操作来增加全局或单个连接的流量计数
	// atomic.AddInt64(&globalBytesSent, int64(n))
	return
}

// pipeTraffic 是经过您验证的、健壮的转发函数
func pipeTraffic(wg *sync.WaitGroup, dst io.WriteCloser, src io.ReadCloser) {
	defer wg.Done()
	// 注意：这里的Close调用是正确的，因为io.Copy返回后，无论如何连接都应该被清理
	defer dst.Close()
	defer src.Close()
	...
	tracker := &copyTracker{Writer: dst}
...
func handleDirectTCPIP(ch ssh.Channel, destHost string, destPort uint32, remoteAddr net.Addr) {
	...
	// 使用您历史版本中经过验证的 pipeTraffic 函数进行双向转发
	// 上传: 客户端(ch) -> 目标(destConn)
	go pipeTraffic(&wg, destConn, ch)
	
	// 下载: 目标(destConn) -> 客户端(ch)
	go pipeTraffic(&wg, ch, destConn)
...
func main() {
	...
	if globalConfig.HandshakeTimeout <= 0 { globalConfig.HandshakeTimeout = 1 }
	if globalConfig.ConnectUA == "" { globalConfig.ConnectUA = "26.4.0" }
	if globalConfig.BufferSizeKB <= 0 { globalConfig.BufferSizeKB = 8 } // 保持您测试有效的小缓冲区
